 ...
 */
export async function detectPackageManagerWithFallback(cwd) {
    try {
        // For the first attempt, use pak's default of requiring a lockfile. This makes sure we find ioBroker's root dir
        return await detectPackageManager(typeof cwd === 'string'
 ...
 */
export async function installNodeModule(npmUrl, options = {}) {
 ...
 * @param options Options for the installation
 */
export async function uninstallNodeModule(packageName, options = {}) {
    // Figure out which package manager is in charge (probably npm at this point)
    const pak = await detectPackageManagerWithFallback(options.cwd);
    // By default, don't print all the stuff the package manager spits out
    if (!options.debug) {
 ...
 */
export async function rebuildNodeModules(options = {}) {
    // Figure out which package manager is in charge (probably npm at this point)
    const pak = await detectPackageManagerWithFallback(options.cwd);
    // By default, don't print all the stuff the package manager spits out
    if (!options.debug) {
 ...
 *        </code></pre>
 */
export function generateDefaultCertificates() {
    const pki = forge.pki;
 ...
 * @param objects db
 */
export async function getHostInfo(objects) {
    if (!diskusage) {
        try {
 ...
 */
export function getControllerDir() {
...
export function isDevServerInstallation() {
 ...
 */
export function promisify(fn, context, returnArgNames) {
    return function () {
 ...
 * Otherwise, the Promise will resolve with an array
 */
export function promisifyNoError(fn, context, returnArgNames) {
    return function () {
        // eslint-disable-next-line prefer-rest-params
        const args = sliceArgs(arguments);
        // @ts-expect-error we cannot know the type of `this`
 ...
 */
export function appendStackTrace(str) {
 ...
 */
export function measureEventLoopLag(ms, cb) {
    let start = hrtime();
 ...
 */
export async function getAllInstances(adapters, objects) {
    const instances = [];
 ...
 * @returns Promise
 */
export async function getAllEnums(objects) {
    const allEnums = {};
 ...
 */
export async function getInstances(adapter, objects, withObjects) {
 ...
 */
export function execAsync(command, execOptions) {
 ...
 */
export function pipeLinewise(input, output) {
 ...
 */
export async function isAdapterEsmModule(adapter) {
 ...
 */
export async function resolveAdapterMainFile(adapter) {
    const adapterDir = getAdapterDir(adapter);
    if (!adapterDir) {
        throw new Error(`Could not find adapter dir of ${adapter}`);
    }
 ...
 */
export function getDefaultRequireResolvePaths(callerModule) {
    const ret = [
 ...
 */
export function isShortGithubUrl(url) {
 ...
 * @param url The URL to parse
 */
export function parseShortGithubUrl(url) {
 ...
 */
export function isGithubPathname(pathname) {
 ...
 * @param pathname The pathname part of a GitHub URL
 */
export function parseGithubPathname(pathname) {
 ...
 */
export function removePreservedProperties(preserve, oldObj, newObj) {
 ...
 */
export function getInstanceIndicatorObjects(namespace) {
 ...
 */
export async function setExecutableCapabilities(execPath, capabilities, modeEffective, modePermitted, modeInherited) {
 ...
 */
export function compressFileGZip(inputFilename, outputFilename, options = {}) {
 ...
 */
export function maybeArrayToString(maybeArr) {
 ...
 * @param ip ipv4 or ipv6 address
 */
export function isListenAllAddress(ip) {
 ...
 */
export function ensureDNSOrder() {
    const dnsOrder = getDNSResolutionOrder();
 ...
 */
export function getHostObject(oldObj) {
 ...
 */
export function getPidsFileName() {
 ...
 */
export async function getControllerPid() {
